...
describe('Client Class Integration Tests', () => {
    ...
    it('should subscribe to data using client.subscribe and receive updates', (done) => {
        ...
        const newName = `Subscribed Update ${uuidv4()}`;
        let receivedInitialData = false;
        let receivedUpdate = false;
        let subscription: Subscription | null = null;

        // --- Increase Test Timeout ---
        // Note: done callback implicitly handles timeout errors now, 
        // but we also increase the Jest timeout specifically for this test.
        // jest.setTimeout(35000); // Set timeout specifically for this test (alternative to 3rd arg)

        Debug('  -> Initiating client.subscribe...');
        const observable = client.subscribe<{ users_by_pk: { id: string; name: string } }>({ 
            table: 'users',
            pk_columns: { id: testUserId! },
            returning: ['id', 'name']
            // role: 'user' // Add role if needed, check WSLink notes
        });

        subscription = observable.subscribe({
            next: (result: FetchResult<{ users_by_pk: { id: string; name: string } }>) => {
                Debug(`  📬 [SUB NEXT] Received data: ${JSON.stringify(result)}`); // Log entire result
                if (result.errors) {
                    Debug(`  ❌ [SUB NEXT] GraphQL errors received: ${JSON.stringify(result.errors)}`);
                    // Potentially stop test early on error in next?
                    // subscription?.unsubscribe();
                    // done(new ApolloError({ graphQLErrors: result.errors }));
                    // return;
                }
                // It's possible to receive null data even without errors in subscriptions
                if (!result.data || !result.data.users_by_pk) {
                     Debug(`  ⚠️ [SUB NEXT] Received null or undefined data payload.`);
                     // Continue listening for potential updates
                     return;
                }
                ...
                if (!receivedInitialData) {
                    ...
                    Debug('  👍 [SUB NEXT] Received initial subscription data.');

                    // After receiving initial data, trigger an update
                    Debug('  🚀 Triggering update for subscription test...');
                    client.update({ // Use client.update to trigger change
                        table: 'users',
                        pk_columns: { id: testUserId! },
                        _set: { name: newName },
                        returning: ['id'] // Request only id to confirm update went through
                    }).then(updateResult => {
                        Debug(`  ✅ Update mutation successful (Trigger): ${JSON.stringify(updateResult)}`);
                    }).catch(err => {
                         Debug(`  ❌ Error triggering update during subscribe test: ${err}`);
                         subscription?.unsubscribe();
                         done(err); // Fail test if update fails
                    });
                } else if (result.data?.users_by_pk?.name === newName) {
                    receivedUpdate = true;
                    Debug('  🎉 [SUB NEXT] Received updated subscription data!');
                    subscription?.unsubscribe(); // Clean up subscription
                    Debug('  🔌 Subscription unsubscribed.');
                    done(); // Signal test completion
                } else {
                    Debug(`  ⏳ [SUB NEXT] Received data, but name (${result.data.users_by_pk.name}) doesn't match initial (${testUserName}) or updated (${newName}).`);
                }
            },
            error: (err) => {
                Debug(`  ❌ [SUB ERROR] Subscription error handler triggered: ${err}`);
                if (err instanceof Error) {
                    Debug(`  ❌ [SUB ERROR] Stack: ${err.stack}`);
                }
                subscription?.unsubscribe();
                done(err); // Fail test on error
            },
            complete: () => {
                 Debug('  🏁 [SUB COMPLETE] Subscription completed callback triggered.');
                 // This usually shouldn't happen in this test unless the connection is closed
                 if (!receivedUpdate) {
                     done(new Error('Subscription completed before receiving the expected update.'));
                 }
            }
        });

        // Timeout for the test itself using the third argument to `it`
    }, 35000); // Increased timeout to 35 seconds

    // --- Direct Apollo Client Subscription Test ---
    it('[DIAGNOSTIC] should subscribe directly via adminApolloClient', (done) => {
        Debug('\n🧪 Testing direct adminApolloClient.subscribe');
        expect(testUserId).toBeTruthy();

        const newName = `Direct Sub Update ${uuidv4()}`;
        let receivedInitialData = false;
        let receivedUpdate = false;
        let subscription: Subscription | null = null;

        const SUBSCRIBE_QUERY = gql`
            subscription DirectUserSubscription($id: uuid!) {
                users_by_pk(id: $id) {
                    id
                    name
                }
        ...
        Debug('  -> Initiating adminApolloClient.subscribe...');
        const observable = adminApolloClient.subscribe<{
            users_by_pk: { id: string; name: string } | null;
        }>({ 
            query: SUBSCRIBE_QUERY,
            variables: { id: testUserId! },
            fetchPolicy: 'network-only' // Ensure it tries to get fresh data
        });

        subscription = observable.subscribe({
            next: (result) => { // Simplified type here
                Debug(`  📬 [DIRECT SUB NEXT] Received data: ${JSON.stringify(result)}`); 
                if (result.errors) {
                    Debug(`  ❌ [DIRECT SUB NEXT] GraphQL errors received: ${JSON.stringify(result.errors)}`);
                }
                if (!result.data || !result.data.users_by_pk) {
                     Debug(`  ⚠️ [DIRECT SUB NEXT] Received null or undefined data payload.`);
                     return;
                }
                ...
                if (!receivedInitialData) {
                    expect(result.data?.users_by_pk?.name).toBe(testUserName); // Expect original name first
                    receivedInitialData = true;
                    Debug('  👍 [DIRECT SUB NEXT] Received initial subscription data.');

                    Debug('  🚀 Triggering update for DIRECT subscription test...');
                    const UPDATE_USER_NAME = gql`
                        mutation UpdateUserNameDirect($id: uuid!, $name: String!) {
                            update_users_by_pk(pk_columns: {id: $id}, _set: {name: $name}) { id name }
                        }
                    `;
                    adminApolloClient.mutate({ // Use admin client to update
                        mutation: UPDATE_USER_NAME,
                        variables: { id: testUserId!, name: newName }
                    }).then(updateResult => {
                        Debug(`  ✅ Update mutation successful (DIRECT Trigger): ${JSON.stringify(updateResult.data)}`);
                    }).catch(err => {
                         Debug(`  ❌ Error triggering update during DIRECT subscribe test: ${err}`);
                         subscription?.unsubscribe();
                         done(err); 
                    });
                } else if (result.data?.users_by_pk?.name === newName) {
                    receivedUpdate = true;
                    Debug('  🎉 [DIRECT SUB NEXT] Received updated subscription data!');
                    subscription?.unsubscribe(); 
                    Debug('  🔌 DIRECT Subscription unsubscribed.');
                    done(); 
                } else {
                     Debug(`  ⏳ [DIRECT SUB NEXT] Received data, name (${result.data.users_by_pk.name}) mismatch.`);
                }
            },
            error: (err) => {
                Debug(`  ❌ [DIRECT SUB ERROR] Subscription error: ${err}`);
                 if (err instanceof Error) {
                    Debug(`  ❌ [DIRECT SUB ERROR] Stack: ${err.stack}`);
                }
                subscription?.unsubscribe();
                done(err); 
            },
            complete: () => {
                 Debug('  🏁 [DIRECT SUB COMPLETE] Subscription completed.');
                 if (!receivedUpdate) {
                     done(new Error('[DIRECT] Subscription completed before receiving update.'));
                 }
            }
        });
    }, 35000); // Timeout for direct subscription test
}); 